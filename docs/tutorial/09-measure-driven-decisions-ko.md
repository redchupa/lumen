# 9장. 측정-주도 의사결정 — 시공 전 지반 측량부터 하기

> 이 장에서는 이 책의 전반에서 자주 등장한 "측정으로 부정", "11번 중 8번 부정"이
> 정확히 어떤 사이클이고 왜 그 패턴이 사이드 프로젝트의 본질인지 다룹니다. 도시
> 건축으로 풀면 **"건물 짓기 전에 지반 측량부터 하기"**예요. 직관으로 보면 단단해
> 보이는 땅이 측량해 보니 약지반인 경우가 흔합니다. Lumen v0.5 cycle의 11번 가설
> 을 하나씩 살펴보고, 측정 인프라를 어떻게 짜야 그 사이클이 가능한지까지.
...
도시 비유로 풀면 — 건물 하나 짓기 전에 다음 순서:

1. 가설 명시: "이 땅에는 5층 빌딩 지을 수 있을 거다"
2. 측정 도구 준비: 지반 측량기, 토양 시료 채취
3. 시범 시공 또는 측량
4. 측정: 지반 강도 결과
5. 결정: 5층 가능 → 본격 시공 / 약지반 → 다른 설계 또는 다른 땅으로

순서를 거꾸로 하면(시공하고 측량) 무너진 건물부터 봐야 해요. 막대한 손실.
...
도시 비유로 풀면 — 첫 건물 짓기 전에 측량기부터 사 두는 거예요. 측량기 없이 시공
시작하면 6주 후에 건물 무너지고 나서야 "지반이 문제였구나" 알게 돼요.
...
도시 비유로 풀면 — "철근 단가가 싸니까 철근 더 쓰면 시공이 빨라질 것"이라는 가설.
측정해 보니 단가는 싸진했는데 시공 시간 영향은 미미. 진짜 시간 절약은 다른 단계
에 있다는 시그널.
...
도시 비유로 풀면 — "강력한 다목적 시공 장비" 도입을 측량했더니 일반 시공에는
오버스펙. 그런데 1년 후 큰 빌딩 시공할 때 그 장비가 진가 발휘.
...
**왜**: 5장에서 본 그 사실. AMD Zen 4는 AVX-512를 내부적으로 256비트 두 번으로
쪼개서 실행. 즉 ZMM 명령이 두 µop으로 분해돼서 사실상 YMM 두 번과 같음. 처리량은
같은데 인코딩 오버헤드와 dependency 체인이 길어져서 손해.
...
도시 비유로 풀면 — "신소재 자재가 기존 자재보다 2배 강하다"고 측정했더니 우리
지역 기후에서는 오히려 변형. 알고 보니 그 신소재는 다른 기후에서만 진가 발휘. 다른
지역에서 시공할 때 다시 측정.
...
도시 비유로 풀면 — "다음 자재 미리 보내기" 신호가 1개 공사팀에서는 win인데 8개
공사팀이 동시에 신호 보내면 자재 도로가 혼잡해서 모든 자재가 지연. 같은 신호가
공사팀 수에 따라 정반대 효과.
...
도시 비유로 풀면 — 건축 책의 일반 원리(예: "철근 많이 쓰면 강하다")는 그 지역의
특수 조건(지반·기후·자재 공급)을 만나면 변형돼요. 그 변형을 책에 다 적어 둘 수
없어요. 현장에서 측량해야 합니다.
...
도시 비유로 풀면 — 건설 회사 차리면 사무실 가구보다 측량기부터 사세요. 측량기
없이 시공 시작하면 첫 건물부터 무너집니다.
...
도시 비유로 풀면 — 건물 완공까지 6개월 걸렸을 때 "왜 오래 걸렸나" 분석하려면 각
단계(기초·골조·전기·인테리어) 시간을 따로 측정해야 함. 전체 6개월만 알면 다음
건물 어디 손대야 할지 모름.
...
**폐기 (revert)**: 코드 다 되돌리기. 흔적 없음. 다음에 같은 분야 다시 손대면 처음
부터.

**default-off**: 코드는 main에 살리되 기본값을 off로. flag나 env var로 한 줄 변경
하면 활성화 가능.
...
도시 비유로 풀면 — "이 신소재는 우리 지역 기후에서 성능 안 나옴"을 측정으로 확인.
그렇다고 신소재 매뉴얼 다 폐기하지 말고 보관. 다른 지역 시공 들어갈 때 다시 검토
가능. 다음 측량 비용 0, 매뉴얼 보관 비용 0.
...
이 받아들임이 사이드 프로젝트 5주 이상 지속의 핵심이에요. 매 부정을 손해로 받으면
6주째 흐지부지. 매 부정을 데이터로 받으면 다음 phase 진입이 자연스러움.

## 9.7 사이드 프로젝트 시작하시는 분께

이 책에서 사이드 프로젝트 한 사이클을 다 풀었어요. 같은 분야든 다른 분야든 시작
하시려는 분께 정리.

**1. 첫 commit 다음이 측정.** "Hello World" 다음에 "Hello World 시간 측정". 측정
없이 5주 가면 흐지부지. 건설 회사 차리면 측량기부터.
...
1. **측정-주도 의사결정의 사이클**: 가설 → 측정 인프라 확인 → 변경 → 측정 → 결정.
   한 사이클 1~3일. **건축으로는 "시공 전 지반 측량"의 순서**.
...
3. **직관은 마이크로아키텍처 분야에서 자주 틀린다**. ZMM lane width, prefetch,
   batching 등 책의 상식이 실제 호스트에서는 부정. 책의 일반 원리가 그 지역의
   특수 조건을 만나면 변형됨.

4. **인프라 폐기보다 default-off**. 미래 다른 호스트/조건에서 살아날 수 있음.
   7.O의 7.P 부활이 증거. **"신소재 매뉴얼은 다른 지역에서 다시 검토 가능"**.
...
실제 일기. 측정 데이터, 가설, 부정 사례, win, 그리고 무엇을 배웠는지를 시간 순
으로 정리합니다. 이 장을 읽고 나면 "내 사이드 프로젝트도 같은 패턴으로 가능
하다"가 보일 거예요.
